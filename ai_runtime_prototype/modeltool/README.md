...
If the bundle includes `model_profile.json`, every reference input and optional
reference output tensor is checked locally before conversion: each name, dtype
and static shape must match the profile, and floating tensors must contain only
finite values. A bundle without reference inputs is accepted for inspection,
but reported as a **warning**; a PyTorch conversion then needs the explicit
`--allow-synthetic-validation` smoke-test override. Production releases should
always include real, preprocessed reference inputs, and should include
`reference_outputs` when the notebook can export golden logits/probabilities.
