...
{
    if (runtime == NULL || runtime->magic != DL_RUNTIME_MAGIC || runtime != g_runtime_owner) {
        return;
    }
    runtime->magic = 0;
                                 ...
                                 dl_benchmark_result_t *result)
{
    ...
    for (int i = 0; i < config->warmup_runs; ++i) {
        dl_status_t status = dl_runtime_execute(runtime, inputs, input_tensor_count,
                                                outputs, output_tensor_count, NULL);
    ...
    for (int i = 0; i < config->measured_runs; ++i) {
        dl_status_t status = dl_runtime_execute(runtime, inputs, input_tensor_count,
                                                outputs, output_tensor_count, &samples[i]);
        if (status != DL_OK) {
            free(samples);
            return status;
        }
    ...
    result->max_ms = samples[config->measured_runs - 1];
